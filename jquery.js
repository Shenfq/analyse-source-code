...
var
	...
	_$ = window.$,   //用于防止命名冲突
	//下面的这三个变量其实代表了三种不同的数据类型，对象、数组、字符串。
	// [[Class]] -> type pairs
	class2type = {},   //该对象存储了对象的几种不同类型，用来进行具体对象类型的判断，参见429行
	...
	// Save a reference to some core methods  将常用方法存储，可用来方法的借用
	...
	// Define a local copy of jQuery    初始化一个jquery对象，返回一个init对象的实例
...
jQuery.fn = jQuery.prototype = {//将jquery的原型对象赋给jquery.fn
	// The current version of jQuery being used
	jquery: core_version, //定义了jquery的版本号

	constructor: jQuery,//修改构造器的指向，指向jquery对象
	init: function( selector, context, rootjQuery ) {
		var match, elem;

		// HANDLE: $(""), $(null), $(undefined), $(false)  传入的selector为空时
		if ( !selector ) {
			return this;
		}

		// Handle HTML strings  传入的selector为字符串时
		if ( typeof selector === "string" ) {
			...
			if ( match && (match[1] || !context) ) {   //如果是字符串是html标签
				...
				// HANDLE: $(#id)
				} else {   //如果该字符串是表示一个id，则通过getElementById js原生方法来获得dom元素
			...
			// HANDLE: $(expr, $(...))    如果该字符串是复杂选择器就调用find方法  即调用 Sizzle 
		...
		// HANDLE: $(DOMElement)   如果selector是一个DOM结点
		...
		// HANDLE: $(function)
		// Shortcut for document ready  如果传入的selector是一个函数  --> $(document).ready( fn )
		...
		return jQuery.makeArray( selector, this );  //如果是其他任意值，抵用makeArray转换为数组
	},
...
jQuery.fn.init.prototype = jQuery.fn;   //让jquery.fn.init的原型继承自jquery的原型
...
jQuery.extend({
	// Unique for each copy of jQuery on the page   	生成唯一的jquery字符串
	...
	isReady: false, //该属性用来判断当前文档的DOM树是否已经加载完毕 

	// A counter to track how many items to wait for before
	// the ready event fires. See #6781
	readyWait: 1, //readyList队列中的等待调用的函数的数量
	...
	isFunction: function( obj ) {  //判断传入的一个对象是否为一个函数
	...
	type: function( obj ) {  //用来判断数据类型，如果是object则通过 toString 来判断类型，包括array date regexp error等
		if ( obj == null ) {
			return String( obj );
		}
	...
	isPlainObject: function( obj ) {   //判断是否为对象自变量
		// Not plain objects:
	...
	isEmptyObject: function( obj ) {   //判断是否为一个空对象
		var name;
		for ( name in obj ) {
			return false;
		}
	...
	parseHTML: function( data, context, keepScripts ) {   //解析html字符串，并创建dom节点
	...
	parseJSON: JSON.parse,   //直接调用原生的 JSON 对象来解析json字符串
	...
	camelCase: function( string ) {//转成驼峰命名法
	...
	// args is for internal usage only    对数组或者对象进行遍历
...
// Populate the class2type map     判断obj的具体类型
...
jQuery.Callbacks = function( options ) {
	...
	var // Last fire value (for non-forgettable lists)
		...
		list = [], 
		// Stack of fire calls for repeatable lists   一个可重复被激活的队列
		stack = !options.once && [],
		// Fire callbacks
		fire = function( data ) {
		...
		self = {
			...
			add: function() {  // 将一个回调函数添加到list队列
				if ( list ) {
					...
					} else if ( memory ) {  //如果callbacks对象是memory类型的，传入的回调函数直接被激活
						firingStart = start;
						fire( memory );
					}
				}
			...
			fireWith: function( context, args ) {   
				if ( list && ( !fired || stack ) ) {
					args = args || [];
					args = [ context, args.slice ? args.slice() : args ];   //args.slice ?
					if ( firing ) {
						stack.push( args );
					} else {
						fire( args );
...
jQuery.extend({
	...
	Deferred: function( func ) {
		var tuples = [	//Deferred对象提供了三种状态。成功、失败、进行
		...
		// Make the deferred a promise   将promise上的方法扩展到deferred
...
function Data() {  //创建一个Data构造函数
	// Support: Android < 4,
	// Old WebKit does not have Object.preventExtensions/freeze method,
	// return new empty object instead with no [[set]] accessor
	Object.defineProperty( this.cache = {}, 0, {  //当访问Data.cache[0]时，默认返回一个空对象，且不能被修改。
		get: function() {
			return {};
		}
	});
...
Data.accepts = function( owner ) {  
	// Accepts only:  //除以下几种情况外返回false;
	//  - Node   owner具有nodeType属性，且是Element或document
	//    - Node.ELEMENT_NODE
	//    - Node.DOCUMENT_NODE
	//  - Object  以及不具有nodeType的任何对象
	//    - Any
...
Data.prototype = {
	key: function( owner ) {
		...
		if ( !Data.accepts( owner ) ) {  //如果该对象不能满足Data.accepts条件则return 0
			return 0;
		}
		...
		// Ensure the cache object   确保缓存键创建成功
	...
	access: function( owner, key, value ) {  //该方法是对get方法与set方法的抽象
		...
		//   2. A string key was specified, but no value provided 指定key(字符串),但是没有指定value
		//
		...
		//
		//   1. An object of properties   key为一个对象
		//   2. A key and value   同时存在key和value
		//
	...
	remove: function( owner, key ) {
		...
		if ( key === undefined ) {//如果不指定要删除的key名，则会将当前缓存对象置空
			...
			// Support array or space separated string of keys  如果有key时：
...
// These may be used throughout the jQuery core codebase  创建了两个Data()对象的实例
